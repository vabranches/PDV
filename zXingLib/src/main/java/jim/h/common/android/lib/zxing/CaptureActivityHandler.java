...
public final class CaptureActivityHandler extends Handler {
	...
	public void handleMessage(Message message) {
		if (message.what == R.id.zxinglib_auto_focus) {
			...
			CameraManager.get()
					.requestAutoFocus(this, R.id.zxinglib_auto_focus);
	 ...
	 *           (message.what) { case R.id.zxinglib_auto_focus: //Log.d(TAG,
	 *           "Got auto-focus message"); // When one auto focus pass
	 *           finishes, start another. This is the closest thing to //
	 *           continuous AF. It does seem to hunt a bit, but I'm not sure
	 *           what else to do. if (state == State.PREVIEW) {
	 *           CameraManager.get().requestAutoFocus(this,
	 *           R.id.zxinglib_auto_focus); } break; case
	 *           R.id.zxinglib_decode_succeeded: Log.d(TAG,
	 *           "Got decode succeeded message"); state = State.SUCCESS; Bundle
	 *           bundle = message.getData(); Bitmap barcode = bundle == null ?
	 *           null : (Bitmap) bundle
	 *           .getParcelable(DecodeThread.BARCODE_BITMAP);
	 *           activity.handleDecode((Result) message.obj, barcode); break;
	 *           case R.id.zxinglib_decode_failed: // We're decoding as fast as
	 *           possible, so when one decode fails, start another. state =
	 *           State.PREVIEW;
	 *           CameraManager.get().requestPreviewFrame(decodeThread
	 *           .getHandler(), R.id.zxinglib_decode); break; case
	 *           R.id.zxinglib_return_scan_result: Log.d(TAG,
